...
fn maths_entries(config: &ConfigManager) -> Vec<MenuEntry> {
	commands::menu_entries(&[ActionId::PreviousMath, ActionId::NextMath], config)
}
...
fn create_maths_submenu(config: &ConfigManager) -> Menu {
	let entries = maths_entries(config);
	build_menu(&entries)
}

fn append_maths_items(menu: &Menu, config: &ConfigManager) {
	let entries = maths_entries(config);
	append_menu_entries(menu, &entries);
}
...
pub fn create_go_menu(config: &ConfigManager, compact: bool) -> Menu {
	...
	if compact {
		...
		menu.append_submenu(create_maths_submenu(config), &maths_label, &maths_help);
		...
		menu.append_separator();
		append_maths_items(&menu, config);
		menu.append_separator();
